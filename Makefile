...
OBJ+=facenet_test.o run_darknet.o face_io.o test.o
ifeq ($(GPU), 1) 
LDFLAGS+= -lstdc++ 
...
DEPS += $(wildcard src/*.h) Makefile darknet_src/include/darknet.h

all: weights data model obj $(EXEC) $(LIBNAMESO) $(APPNAMESO)
...
data:
	mkdir -p data; touch data/name

weights:
	cd /tmp; wget https://www.dropbox.com/s/niie8a9p6otve9g/facenet-weights.tgz
	tar xvfz /tmp/facenet-weights.tgz
...
reset:
	rm -rf $(OBJS) $(EXEC) $(LIBNAMESO) $(APPNAMESO) obj data model weights
